            ...
            <div class="btn-toolbar mb-2 mb-md-0">
                <div class="btn-group mr-2">
                    @if($user->id==$company->admin_id)
                        <a href="{{ route('userRequest',['id'=>$company->id]) }}" role="button"
                           class="btn btn-sm btn-outline-success">Заявки {{count($company->rqusers)}}</a>
                    @endif
                    <a href="{{route('companyUsers', ['companyId'=>$company->id])}}"
                       class="btn btn-sm btn-outline-success" role="button">Сотрудники {{count($company->users)}}</a>
                    <a href="{{route('idea', ['companyId'=>$company->id])}}" class="btn btn-sm btn-outline-success"
                       role="button">Идеи сообщества {{count($company->ideas)}}</a>
                    <a href="" class="btn btn-sm btn-outline-secondary disabled" role="button"> Цепочки точек</a>
                </div>
            </div>
            <div class="btn-toolbar mb-2 mb-md-0">
                <div class="btn-group mr-2">
                    <a href="{{ route('companyEdit',['companyId'=>$company->id]) }}"
                       class="btn btn-sm btn-outline-success" role="button">Редактировать компанию</a>
                    <a href="{{ route('companyProfile',['companyId'=>$company->id]) }}"
                       class="btn btn-sm btn-outline-success" role="button">Профиль</a>
                    ...
                    <div class="row">
                        <div class="col-sm">
                            <button type="button" class="btn btn-success btn-sm"
                                    onClick="addDotFromCompanyIndex({{ $company->id }}, {{ $user->id }}, 0)"
                    ...
                    <div class="row">
                            ...
                            @foreach($mainDots as $dot)
                                <div class="col-md-4 mt-2">
                                    <div class="card shadow-sm h-100">
                                        <h6 class="card-header"><img src="{{ $dot->logo }}" class="mr-2 rounded-circle"
                                                                     alt="" height="40" width="40"
                                                                     align="cover-container">
                                            <a href="{{ route('dotIndex',['id'=>$company->id, 'dotId'=>$dot->id]) }}">{{ $dot->name }}</a>
                                            @if (Auth::User()->id==$company->admin_id)
                                                <button type="button" class="ml-2 mb-1 close"
                                                        onclick="delDot({{$dot->id}}, {{$company->id}})"
                        ...
                        @else
                            <div class="alert alert-info" role="alert">
                                У этой компании нет ни одной точки контакта? Не может быть! <strong>Добавьте
                                    первую!</strong>
                            </div>
                        @endif
                    </div>
                ...
                <div class="row">
                    <div class="col-sm">
                        <button type="button" href="#"
                                onClick="addTaskFromCompanyIndex({{ $company->id }}, {{ $user->id }})"
